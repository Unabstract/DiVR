...
class crossmodal_net_graph_het(nn.Module):
    def __init__(self, config):
        ...
        self.GCN = TemporalGCNGraphEmbedding_het(self.num_node_features,
                                                 self.num_edge_features,
                                                 self.hidden_channels,
                                                 self.out_channels,
                                                 self.config.input_seq_len)

        self.graph_encoder = PerceiveEncoder(n_input_channels=config.graph_hidden_dim,
                                            n_latent=output_len,
                                            n_latent_channels=config.graph_latent_dim,
                                            n_self_att_heads=config.graph_n_heads,
                                            n_self_att_layers=config.graph_n_layers,
                                            dropout=config.dropout)             # (bs, out_len, gaze_latent_d)

        self.motion_graph_decoder = PerceiveDecoder(n_query_channels=config.graph_latent_dim,
                                                       n_query=output_len,
                                                       n_latent_channels=config.motion_latent_dim,
                                                       dropout=config.dropout)  # (bs, out_len, gaze_latent_d)

    def forward(self, gazes, gazes_mask, motions, smplx_vetices, scenes, graphs):
        """
        ...
        if self.use_gaze:
            ...
            # gaze_embedding = (torch.ones((bs, seq_len, self.config.scene_feats_dim))*10).float().to(self.device)
        ...
        # graph embedding
        graph_embedding = self.GCN(graphs)

        graph_embedding = self.graph_encoder(graph_embedding)

        graph_motion_embedding = self.motion_graph_decoder(motion_embedding, graph_embedding)

        cross_modal_embedding = graph_motion_embedding
...
if __name__ == '__main__':
    ...
    net = crossmodal_net_graph_het(configs).cuda()
